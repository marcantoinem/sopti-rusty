use crate::components::common::schedule::Schedule;
use leptos::{ev, prelude::*};
use web_sys::wasm_bindgen::JsCast;
use web_sys::Element;

fn minmax(x: i32, y: i32) -> [i32; 2] {
    if x < y {
        [x, y]
    } else {
        [y, x]
    ...
    F: Fn() + Copy + 'static,
{
    ...
    let selection = move || {
        ...
        let [initial_x, destination_x] = minmax(initial_x, destination_x);
        let [initial_y, destination_y] = minmax(initial_y, destination_y);
    ...
    let handle = window_event_listener(ev::pointerup, move |_| {
        ...
        let [initial_x, destination_x] = minmax(initial_x, i);
        let [initial_y, destination_y] = minmax(initial_y, j);
        let is_positive = is_positive.get();
        for day in initial_x..=destination_x {
            week[day as usize].update(|v| {
                if is_positive {
                            ...
                            view! {
                                <div
                                    ...
                                    on:pointerdown=move |e| {
                                        ...
                                        set_positive.set((week[i as usize].get() & (1 << j)) == 0);
                                        let _ = e
                                            .target()
                                            .unwrap()
                                            .dyn_ref::<Element>()
